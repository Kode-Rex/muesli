...
final class PerformanceMonitor: ObservableObject {
    ...
    func generatePerformanceReport() -> String {
        ...
        • Average Duration: \(String(format: "%.2f", dataOpsAvg * 1_000))ms

        Search Operations:
        • Count: \(metrics.searchOperations.count)
        • Average Duration: \(String(format: "%.2f", searchOpsAvg * 1_000))ms

        Write Operations:
        • Count: \(metrics.writeOperations.count)
        • Average Duration: \(String(format: "%.2f", writeOpsAvg * 1_000))ms
